...
import swap.SwapUtilsForBucketSort;

public class QuickSortForBucketSort{
	...
	public void quickSort(Integer[] nums, int start, int end){
		...
		SwapUtilsForBucketSort.swap(nums, pivot, end-1);//交换到最后
		...
		while(left<=right){
			if(nums[left] < value){
				...
				SwapUtilsForBucketSort.swap(nums, left, right);
				left++;
				right--;
			}
		}
		SwapUtilsForBucketSort.swap(nums, left, end -1);//将值交换回来
	...
	public Integer[] insertSortForQuickSort(Integer[] nums,int start, int end) {
		//使前i个元素有序 
